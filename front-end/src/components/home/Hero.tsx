...
export default function Hero() {
  const [stats, setStats] = useState<{ caterers_count: number; average_rating: number } | null>(null);
      ...
      <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-12 items-center">
        <div className="relative z-10">
          <h1 className="text-5xl lg:text-7xl font-black leading-[1.1] mb-6">
            Un <span className="text-primary">Traiteur</span> pour chaque occasion
          </h1>
          <p className="text-lg text-neutral-text/80 dark:text-slate-300 mb-10 max-w-xl">
            Connectez-vous aux meilleurs traiteurs locaux. Des dîners intimes aux grands mariages,
            trouvez les saveurs qu'il vous faut.
          </p>
          <SearchBar />
        </div>
        ...
        <div className="relative">
          ...
          {stats && (
            <div className="absolute -bottom-6 -left-6 bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-xl flex items-center gap-4">
              ...
              <div>
                <p className="font-bold text-lg">{stats.average_rating.toFixed(1)}/5 en moyenne</p>
                <p className="text-xs opacity-60">{stats.caterers_count} traiteurs vérifiés</p>
              </div>
            </div>
          )}
        </div>
